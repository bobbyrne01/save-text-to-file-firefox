...
exports.saveTo = function(selectedText){
    ...
    var separator = (Preference.get('format') == 0 ? "\n" : ",");    
	...
	try{        
        ...
        if (Preference.get('datestampInLine')){
        	string += Localisation.getCustomString("datestamp_title") + ': ' + date + separator;
        }
        
        if (Preference.get('timestampInLine')){
        	string += Localisation.getCustomString("timestamp_title") + ': ' + time + separator;
        }
